...
public class Utility {
    ...
    public static boolean handProvinceResponse(String response){
        if(!TextUtils.isEmpty(response)){
            try {
                JSONArray allPrivince=new JSONArray(response);
                for (int i = 0; i <allPrivince.length() ; i++) {
                    JSONObject provinceObject=allPrivince.getJSONObject(i);
